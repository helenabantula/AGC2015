function [FD_score, scoresSTR] = CHALL_AGC15_ComputeDetScores(...
    DetectionSTR, AGC15_Challenge1_STR, show_figures)
%
% Compute face detection score
%
% [FD_score, scoresSTR] = CHALL_AGC15_ComputeDetScores(...
%    DetectionSTR, AGC15_Challenge1_STR, show_figures)
%
% INPUTS
...
%
%   - AGC15_Challenge1_STR: The ground truth structure (e.g.
%   AGC15_Challenge1_EVAL or AGC15_Challenge1_TEST).
%
...
scoresSTR = struct();
for j = 1 : length( AGC15_Challenge1_STR )
    if show_figures
        A = imread( AGC15_Challenge1_STR(j).imageName );    
        
        clf;
        imshow( A, 'InitialMagnification', 'fit');
        hold on;
        
        for k1 = 1 : size( AGC15_Challenge1_STR(j).faceBox, 1 )
            fb = AGC15_Challenge1_STR(j).faceBox( k1, : );
    ...
    n_actualFaces = size( AGC15_Challenge1_STR(j).faceBox, 1 );
    ...
    if not( n_actualFaces )
        ...
        if not( n_detectedFaces )
            ...
            for k1 = 1 : n_actualFaces
                f = AGC15_Challenge1_STR(j).faceBox(k1, :);
